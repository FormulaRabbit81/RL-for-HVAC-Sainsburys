"""We attempt to get the agent to learn in week long episodes, taking the state as time of week.

We assume no dependence between weeks.
"""
...
class EpsilonGreedy:
    ...
    def run(self, alpha, gamma, exttempsdf, pricingdf, cmintemp=17, omintemp=20, maxtemp=22, priceweight=1, comfortweight=1, initialvals=None):
        ...
        def findstate(tempin, tempout, timestep):
            ...
            return [outtemp, intemp, day, hour]
        ...
        timesselected = np.zeros((discretisetemp(exttempsdf), tempstates, 7, 24, tempstates))
        qtable = initialvals if initialvals is not None else np.zeros((discretisetemp(exttempsdf), tempstates, 7, 24, tempstates))
        qtable[:, 0:5, 6:22, 8] += 1
        qtable[:, 6, 9:16, 8] += 1
        print(qtable.shape)
        t_sensors = [19] * 13
        ...
        newstate = findstate(tempin, exttempsdf.iloc[0, 0], 0)
        ...
        for t in range(len(exttempsdf) - 1):
            ...
            if 6 < (t + 1) % 24 < 23 and t % 168 < 144:
                ...
                open = 0
            newstate = findstate(tempin, exttempsdf.iloc[t + 1, 0], t + 1)  # Observe S'
...
def addnoise(num, var):
    return num + np.random.normal(scale=var)
